	...
	for count in counter_array:
		...
		for i in range(len(dati)):
			...
			if imagetype!='xy' and np.mean(target_img**2)>1e-10:
				...
				# plt.imshow(input_img, cmap='Reds', aspect='auto')
				# plt.xlabel('X [m]'), plt.ylabel('Y [m]')
				# plt.grid(None)
				# plt.subplot(122), plt.title('Target xy image')
				# plt.imshow(target_img, cmap='Reds', aspect='auto')
				# plt.xlabel('X [m]'), plt.ylabel('Y [m]')
				# plt.grid(None)
				# plt.show()
...
def main():
	...
	for idx in range(len(Y_test)):
		...
		if args.imagetype=='xy':
			...
			# 					wspace=0.4,
			# 					hspace=0.4)
			plt.subplot(131), plt.title('Target')
			plt.imshow(np.clip(ground_truth, a_min=0, a_max=0.1)/0.1, cmap='Reds', aspect='auto')
			plt.xlabel('Freq [Hz]'), plt.ylabel('X [m]')
			plt.grid(None)
			plt.subplot(132), plt.title('U-net input')
			plt.imshow(np.clip(down, a_min=0, a_max=0.1)/0.1, cmap='Reds', aspect='auto')
			plt.xlabel('Freq [Hz]')
			plt.grid(None)
			plt.subplot(133), plt.title('Prediction')
			plt.imshow(np.clip(prediction, a_min=0, a_max=0.1)/0.1, cmap='Reds', aspect='auto'), plt.colorbar()
			plt.xlabel('Freq [Hz]')
			plt.grid(None)
			plt.show()
